import { BookOpenIcon, SearchCircleIcon, SearchIcon } from '@heroicons/react/outline';
    ...
    >
      ...
      <section className='flex items-center justify-between space-x-3 px-4'>
        <div className=' w-full px-2 py-1 flex items-center space-x-2 bg-[#EBECF0] rounded-md'>
          <SearchCircleIcon className='h-5 w-5 text-zinc-800' />
          <input
            type='search'
      ...
      <section className='px-8'>
        ...
        <div className='space-y-1 overflow-y-auto  divide-y divide-zinc-200'>
          {quickLinks.map(({ icon, name }) => (
            <Link href='/' key={name}>
              <a className='flex items-center space-x-2 py-1 text-zinc-800 hover:bg-apple-gray6'>
                <span>{icon}</span>
                <p>{name}</p>
              </a>
            </Link>
...
const quickLinks = [
  { icon: <BookOpenIcon className='h-5 w-5 text-zinc-700' />, name: 'Matematika' },
  { icon: <BookOpenIcon className='h-5 w-5 text-zinc-700' />, name: 'Fisika' },
  { icon: <BookOpenIcon className='h-5 w-5 text-zinc-700' />, name: 'Analisis' },
