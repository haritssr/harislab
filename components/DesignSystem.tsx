...
export const rumusTitle =
  "font-medium text-md inline-block w-full  text-pink-600";

const thead = "border-t border-gray-300";
const th =
  "w-1/5 py-2 px-2 sm:px-4 text-left font-medium text-gray-700 bg-gray-200 w-max";
...
export const Anchor = ({ to, name }: { to: string; name: string }) => {
  return (
    <Link
      href={to}
      className="text-blue-500 hover:text-fuschsia-500 hover:underline"
    >
      {name}
    </Link>
  );
...
export function InternalLink({ name, to }: { name: string; to: string }) {
  return (
    <Link
      href={to}
      className="flex items-center space-x-2 cursor-pointer text-harislab hover:underline"
    >
      {name}
      <RightArrow />
    </Link>
